...
pub fn macro_group(input: TokenStream) -> TokenStream {
  ...
  let toks =
    group::inner(&ast, false).unwrap_or_else(|err| err.to_compile_error().into());
  toks.into()
}
...
/// #[liberty(group(type=Vec))]
/// ```
#[proc_macro_derive(LinkGroup, attributes(liberty))]
pub fn macro_link_group(input: TokenStream) -> TokenStream {
  let ast = parse_macro_input!(input as DeriveInput);
