...
impl Eq for Cell {}
    ...
    impl crate::ast::HashedGroup for Cell {
        ...
        fn gen_id(
            &self,
            mut title: Vec<String>,
        ) -> Result<Self::Id, crate::ast::IdError> {
            let l = title.len();
            if l != 1 {
    ...
    impl crate::ast::GroupAttri for Cell {
        ...
        fn fmt_liberty<T: std::fmt::Write>(
            ...
            f.write_fmt(
                format_args!(
                    "\n{0} ({1}) {{", key, crate ::ast::HashedGroup::title(self).iter()
                    .map(| s | if crate ::ast::is_word(s) { s.clone() } else { "\""
                    .to_owned() + s + "\"" }).join(",")
                ),
            )?;
            f.indent(1);
                        ...
                        match key {
                            "area" => {
                                ...
                                (
                                    input,
                                    simple_res,
                                ) = <_ as crate::ast::SimpleAttri>::nom_parse(
                                    input,
                                    line_num,
                                )?;
                                match simple_res {
                            ...
                            "pin" => {
                                ...
                                (
                                    input,
                                    group_res,
                                ) = <_ as crate::ast::GroupAttri>::nom_parse(
                                    input,
                                    line_num,
                                )?;
                                match group_res {
                                ...
                                (
                                    input,
                                    n,
                                ) = crate::ast::parser::comment_space_newline(input)?;
                                *line_num += n;
                            }
                            "statetable" => {
                                let group_res: _;
                            ...
                            _ => {
                                ...
                                (
                                    input,
                                    undefined,
                                ) = crate::ast::parser::undefine(input, line_num)?;
                                res.undefined_list().push((key.to_owned(), undefined));
                                let n: usize;
                                (
                                    input,
